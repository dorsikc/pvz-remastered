...
ZombieDefinition gZombieDefs[NUM_ZOMBIE_TYPES] = {
    ...
    { ZOMBIE_TRAFFIC_CONE,      REANIM_ZOMBIE,              2,      1,      1,      4000,   _S("CONEHEAD_ZOMBIE")},
    { ZOMBIE_POLEVAULTER,       REANIM_POLEVAULTER,         2,      6,      5,      2000,   _S("POLE_VAULTING_ZOMBIE")},
    { ZOMBIE_PAIL,              REANIM_ZOMBIE,              4,      4,      1,      3000,   _S("BUCKETHEAD_ZOMBIE")},
    { ZOMBIE_NEWSPAPER,         REANIM_ZOMBIE_NEWSPAPER,    2,      7,      1,      1000,   _S("NEWSPAPER_ZOMBIE")},
    { ZOMBIE_DOOR,              REANIM_ZOMBIE,              4,      13,     5,      3500,   _S("SCREEN_DOOR_ZOMBIE")},
    { ZOMBIE_FOOTBALL,          REANIM_ZOMBIE_FOOTBALL,     7,      13,     5,      2000,   _S("FOOTBALL_ZOMBIE")},
    { ZOMBIE_DANCER,            REANIM_DANCER,              5,      15,     5,      1000,   _S("DANCING_ZOMBIE")},
    { ZOMBIE_BACKUP_DANCER,     REANIM_BACKUP_DANCER,       1,      15,     1,      0,      _S("BACKUP_DANCER")},
    ...
    { ZOMBIE_DOLPHIN_RIDER,     REANIM_ZOMBIE_DOLPHINRIDER, 3,      22,     10,     1500,   _S("DOLPHIN_RIDER_ZOMBIE")},
    ...
    { ZOMBIE_BUNGEE,            REANIM_BUNGEE,              3,      4,      10,     1000,   _S("BUNGEE_ZOMBIE") },
    { ZOMBIE_LADDER,            REANIM_LADDER,              4,      43,     10,     1000,   _S("LADDER_ZOMBIE")},
    { ZOMBIE_CATAPULT,          REANIM_CATAPULT,            5,      46,     10,     1500,   _S("CATAPULT_ZOMBIE")},
    { ZOMBIE_GARGANTUAR,        REANIM_GARGANTUAR,          10,     45,     15,     1500,   _S("GARGANTUAR")},
    { ZOMBIE_IMP,               REANIM_IMP,                 10,     45,     1,      0,      _S("IMP")},
    ...
    { ZOMBIE_JALAPENO_HEAD,     REANIM_ZOMBIE,              3,      22,     10,     1000,   _S("JALAPENO_ZOMBIE")},
...
void Zombie::ZombieInitialize(int theRow, ZombieType theType, bool theVariant, Zombie* theParentZombie, int theFromWave, bool theAnimateBush)
{
    ...
    if (mApp->mImpossiblemode == true) // IMPOSSIBLE MODE SPAWN TIERS
    {
        ZombieDefinition gZombieDefs[NUM_ZOMBIE_TYPES] = {
            { ZOMBIE_NORMAL,            REANIM_ZOMBIE,              1,      1,      1,      4000,   _S("ZOMBIE")},
            { ZOMBIE_FLAG,              REANIM_ZOMBIE,              1,      1,      1,      0,      _S("FLAG_ZOMBIE")},
            { ZOMBIE_TRAFFIC_CONE,      REANIM_ZOMBIE,              1,      3,      1,      4000,   _S("CONEHEAD_ZOMBIE")},
            { ZOMBIE_POLEVAULTER,       REANIM_POLEVAULTER,         2,      6,      5,      2000,   _S("POLE_VAULTING_ZOMBIE")},
            { ZOMBIE_PAIL,              REANIM_ZOMBIE,              3,      8,      1,      3000,   _S("BUCKETHEAD_ZOMBIE")},
            { ZOMBIE_NEWSPAPER,         REANIM_ZOMBIE_NEWSPAPER,    1,      11,     1,      1000,   _S("NEWSPAPER_ZOMBIE")},
            { ZOMBIE_DOOR,              REANIM_ZOMBIE,              3,      13,     5,      3500,   _S("SCREEN_DOOR_ZOMBIE")},
            { ZOMBIE_FOOTBALL,          REANIM_ZOMBIE_FOOTBALL,     5,      16,     5,      2000,   _S("FOOTBALL_ZOMBIE")},
            { ZOMBIE_DANCER,            REANIM_DANCER,              3,      18,     5,      1000,   _S("DANCING_ZOMBIE")},
            ...
            { ZOMBIE_JACK_IN_THE_BOX,   REANIM_JACKINTHEBOX,        1,      31,     10,     1000,   _S("JACK_IN_THE_BOX_ZOMBIE")},
            { ZOMBIE_BALLOON,           REANIM_BALLOON,             2,      33,     10,     2000,   _S("BALLOON_ZOMBIE")},
            { ZOMBIE_DIGGER,            REANIM_DIGGER,              4,      36,     10,     1000,   _S("DIGGER_ZOMBIE")},
            { ZOMBIE_POGO,              REANIM_POGO,                4,      38,     10,     1000,   _S("POGO_ZOMBIE")},
            { ZOMBIE_YETI,              REANIM_YETI,                4,      40,     1,      1,      _S("ZOMBIE_YETI")},
            { ZOMBIE_BUNGEE,            REANIM_BUNGEE,              2,      41,     10,     1000,   _S("BUNGEE_ZOMBIE") },
            { ZOMBIE_LADDER,            REANIM_LADDER,              4,      43,     10,     1000,   _S("LADDER_ZOMBIE")},
            { ZOMBIE_CATAPULT,          REANIM_CATAPULT,            5,      46,     10,     1500,   _S("CATAPULT_ZOMBIE")},
...
void Zombie::PickRandomSpeed()
{
    ...
    {
        mVelX = RandRangeFloat(0.96f, 0.97f);
    }
    else
    {
    ...
    if (mApp->mImpossiblemode == true)
    {
        ...
        {
            mVelX = RandRangeFloat(1.38f, 1.4f);
        }
    }
...
void Zombie::UpdateZombieGargantuar()
{
    ...
    if (mZombiePhase == ZombiePhase::PHASE_GARGANTUAR_THROWING)
    {
        ...
        if (aBodyReanim->ShouldTriggerTimedEvent(0.74f))
        {
            ...
            Zombie* aZombieImp;
            aZombieImp = mBoard->AddZombie(ZombieType::ZOMBIE_IMP, mFromWave);
...
void Zombie::OverrideParticleColor(TodParticleSystem* aParticle)
{
    if (aParticle)
    {
        ...
        {
            aParticle->OverrideColor(nullptr, Color(0, 128, 0, 255));
            aParticle->OverrideExtraAdditiveDraw(nullptr, true);
        }
    }
}
            ...
            if (mApp->mGameScene == GameScenes::SCENE_ZOMBIES_WON)
            {
                ...
                else // by InLiothixie
                {
                        ...
                        mBoard->mBackground == BackgroundType::BACKGROUND_3_POOL || mBoard->mBackground == BackgroundType::BACKGROUND_4_FOG)
                    {
                        ...
                        targetPositionX += BOARD_ADDITIONAL_WIDTH;
                        targetPositionY += BOARD_OFFSET_Y;
                    }
                    ...
                    if (mBoard->StageHasRoof())
                    {
                        ...
                        {
                            mPosX = TodAnimateCurveFloatTime(0, 500, mBoard->mCutScene->mCutsceneTime, mGameOverX, aEdgeX / 1.5f + (BOARD_ADDITIONAL_WIDTH / 2), CURVE_LINEAR);
                            mPosY = TodAnimateCurveFloatTime(0, 500, mBoard->mCutScene->mCutsceneTime, mGameOverY, LAWN_YMIN + 85 + BOARD_OFFSET_Y, CURVE_LINEAR);
                        }
                        else if (mBoard->mCutScene->mCutsceneTime <= 1000)
                        {
                            mPosX = TodAnimateCurveFloatTime(500, 1000, mBoard->mCutScene->mCutsceneTime, aEdgeX / 1.5f + (BOARD_ADDITIONAL_WIDTH / 2), aEdgeX, CURVE_LINEAR);
                            mPosY = TodAnimateCurveFloatTime(500, 1000, mBoard->mCutScene->mCutsceneTime, LAWN_YMIN + 85 + BOARD_OFFSET_Y, targetPositionY - 25, CURVE_LINEAR);
                        }
...
void Zombie::UpdatePlaying()
{
    ...
    if (mPoisonCounter > 0)
    {
        ...
        {
            TakeDamage(15, DAMAGE_HITS_SHIELD_AND_BODY);
        }
    }
    if (mButteredCounter > 0)
    {
...
void Zombie::DrawZombiePart(Graphics* g, Image* theImage, int theFrame, int theRow, const ZombieDrawPosition& theDrawPos)
{
    ...
    {
        g->SetColorizeImages(true);
        g->SetColor(Color(0, 128, 0, anAlpha));
        g->DrawImageMirror(theImage, aDestRect, aSrcRect, aMirror);
...
void Zombie::DrawReanim(Graphics* g, const ZombieDrawPosition& theDrawPos, int theBaseRenderGroup)
{
    ...
    {
        aColorOverride = Color(0, 128, 0, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
