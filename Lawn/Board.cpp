...
Board::Board(LawnApp* theApp)
{
	...
	mFastButton->Resize(798 + 150 + BOARD_ADDITIONAL_WIDTH, 70 - BOARD_OFFSET_Y, IMAGE_FASTBUTTON->mWidth, 46); // CHANGED_#TAG
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN || mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM)
	{
		...
		mMenuButton->Resize(628 + 150 + BOARD_ADDITIONAL_WIDTH, 70 - BOARD_OFFSET_Y, 163, 46); // CHANGED_#TAG
	...
	else
	{
		...
		mMenuButton->Resize(681 + 150 + BOARD_ADDITIONAL_WIDTH, 70 - BOARD_OFFSET_Y, 117, 46); // CHANGED_#TAG
...
void Board::PickZombieWaves()
{
	...
	for (int aWave = 0; aWave < mNumWaves; aWave++)
	{
		...
		if (mApp->mImpossiblemode == true) // IMPOSSIBLE MODE SPAWN POINTS
		{
			aZombiePoints *= 2.5f;
		}
		...
		if (aIsFlagWave)
		{
			...
			if (mApp->mImpossiblemode == true) // IMPOSSIBLE MODE WAVE SPAWN POINTS
			{
				aZombiePoints *= 4.5f;
			}
...
{
	Rect aRect(GetSeedBankExtraWidth() + 456 + 150 + BOARD_ADDITIONAL_WIDTH, 70 - BOARD_OFFSET_Y, Sexy::IMAGE_SHOVELBANK->GetWidth(), Sexy::IMAGE_SEEDBANK->GetHeight()); // CHANGED_#TAG
...
void Board::UpdateCursor()
{
	...
	case GameObjectType::OBJECT_TYPE_CHEAT_TEST_BUTTON:
		aShowFinger = true;
		break;
...
void Board::DrawBackdrop(Graphics* g)
{
	...
	else if (mLevel == 4 && mApp->IsFirstTimeAdventureMode())
	{
		...
		int aWidth = TodAnimateCurve(0, 1070 + BOARD_ADDITIONAL_WIDTH, mSodPosition, 0, 773 + BOARD_ADDITIONAL_WIDTH, TodCurves::CURVE_LINEAR);
...
void Board::DrawLevel(Graphics* g)
{
	...
	int aPosY = 425 + 170 + BOARD_OFFSET_Y;
	if (HasProgressMeter())
	{
...
void Board::DrawTopRightUI(Graphics* g)
{
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
	{
		...
		{
			mMenuButton->mY = TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 0, -50, TodCurves::CURVE_EASE_IN_OUT);
	...
	if (theChar == _S('q'))
	{
		...
		else if (mApp->IsIZombieLevel())
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				mChallenge->IZombiePlaceZombie(ZombieType::ZOMBIE_REDEYE_GARGANTUAR, 6, i);
			}
		}
		else
		{
